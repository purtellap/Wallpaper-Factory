...
public class BackgroundImageFragment extends Fragment {
                    ...
                    case R.id.saveButton: {
                        ...
                        if(selectedLibImg != null){
                            if(v.getId() == R.id.saveButton){
                                setBackgroundImageAsync setBackgroundAsync = new setBackgroundImageAsync(
                                        view, MainActivity.objectDB, selectedLibImg.getImageName());
    ...
    public void startCropper(boolean usesUri, boolean usesCirc, Uri uri){
        ...
        if(usesUri){
            ...
            // if cropping new image and using square, lock to size of phone
            if(!usesCirc){
                activity.setAspectRatio(MainActivity.WIDTH, MainActivity.HEIGHT);
            }
        }

        // BACKGROUND FRAGMENT CROPPER SETTINGS
        if(!usesCirc){
    ...
    public static class setBackgroundImageAsync extends AsyncTask<Void, Void, Void>{

        private View view;
        private AppDatabase database;
        private String fileName;

        setBackgroundImageAsync(View c , AppDatabase db, String fileName){
            this.view = c;
            this.database = db;
        ...
        protected Void doInBackground(Void... voids) {
            ...
            Background background = new Background(0, fileName, view.getContext());
